...
Mantissa.Test.TestLiveForm.StateTrackingRepeatedLiveFormWrapperTestCase = Divmod.UnitTest.TestCase.subclass(
    'Mantissa.Test.TestLiveForm.StateTrackingRepeatedLiveFormWrapperTestCase');
/**
 * Tests for L{Mantissa.LiveForm.StateTrackingRepeatedLiveFormWrapper}.
 */
Mantissa.Test.TestLiveForm.StateTrackingRepeatedLiveFormWrapperTestCase.methods(
    function setUp(self) {
        self.node = document.createElement('span');
        self.node.id = 'athena:123';
        document.body.appendChild(self.node);
    ...
    /**
     * L{Mantissa.LiveForm.StateTrackingRepeatedLiveFormWrapper.gatherInputs}
     * should defer to its child widget's implementation, as well as sticking
     * the C{identifier} initarg into its result.
     */
    function test_gatherInputs(self) {
        var widget = Mantissa.LiveForm.StateTrackingRepeatedLiveFormWrapper(
            self.node, 'form', -42);
        widget.childWidgets.push(
            {gatherInputs: function() {
                return {foo: ['baz']};
            }});
        var result = widget.gatherInputs();
        self.assertIdentical(result['foo'].length, 1);
        self.assertIdentical(result['foo'][0], 'baz');
        self.assertIdentical(result['__repeated-liveform-id__'], -42);
    });
...
Mantissa.Test.TestLiveForm.RepeatedLiveFormWrapperTestCase.methods(
    function setUp(self) {
        self.node = Nevow.Test.WidgetUtil.makeWidgetNode();
        self.widget = Mantissa.LiveForm.RepeatedLiveFormWrapper(self.node, 'formName');
    },
